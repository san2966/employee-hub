-- ============================================================================
-- VPS DB Patch — 2026-06-21
-- Force repair:
--  1) Director > Quotation Manager: Accept / Reject actions can read and update
--     purchase_quotes regardless of broken user_roles links on the VPS.
--  2) Tender Head > Task Manager: assigned tasks sync to Tender Executive by
--     allowing reliable tender_tasks reads/writes and fixing older rows where
--     assigned_to was typed as a display name like "Emp2" instead of the
--     executive login username/email.
-- ============================================================================
...
-- Verification: all should show rows/rules after patch.
SELECT 'purchase_quotes' AS table_name, count(*) FROM public.purchase_quotes;
SELECT 'tender_tasks' AS table_name, count(*) FROM public.tender_tasks;
SELECT username, role FROM public.get_tender_users();
SELECT id, quote_id, status, description FROM public.purchase_quotes ORDER BY created_at DESC LIMIT 10;
SELECT id, assigned_by, assigned_to, task_title, status FROM public.tender_tasks ORDER BY created_at DESC LIMIT 10;