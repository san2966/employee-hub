...
const DirectorAttendance = () => {
  ...
  const { records, counts, pendingCounts, loading, fetchMonthlyRecords } = useAttendanceData(selectedDate);
  ...
  const downloadCSV = (data: any[], filename: string) => {
    const csv = Papa.unparse(data.map(r => ({
      Employee: r.employee_name, Date: r.date, Location: r.location,
      "In Time": r.in_time || "-", "Out Time": r.out_time || "-", Status: r.status,
    })));
  ...
  return (
    <DirectorLayout title="Attendance Manager">
                ...
                {filteredRecords.map(record => {
                  ...
                  return (
                    <TableRow key={record.id}>
                      ...
                      <TableCell>
                        <Badge className={cn(color.bg, color.text, "border-0")}>{color.emoji} {record.location}{record.status === "Pending" && "*"}</Badge>
                      </TableCell>
                      <TableCell>{record.in_time || "-"}</TableCell>
                      <TableCell>{record.out_time || "-"}</TableCell>
                      <TableCell>
                        <Badge variant={record.status === "Approved" ? "default" : record.status === "Rejected" ? "destructive" : "secondary"}>{record.status}</Badge>
                      </TableCell>
                    </TableRow>
                  );
                })}
        ...
        <div className="bg-card rounded-xl border p-6">
          ...
          <div className="flex flex-wrap items-end gap-4">
            ...
            {showMonthly && monthlyRecords.length > 0 && (
              <div className="flex gap-2 ml-auto">
                <Button variant="outline" size="sm" onClick={() => downloadCSV(monthlyRecords, `Monthly_${filterYear}_${filterMonth}.csv`)}>
                  <Download className="h-4 w-4 mr-1" /> CSV
                </Button>
                <Button variant="outline" size="sm" onClick={() => downloadPDF(monthlyRecords, `Monthly - ${format(new Date(filterYear, filterMonth - 1), "MMMM yyyy")}`, `Monthly_${filterYear}_${filterMonth}.pdf`)}>
                  <Download className="h-4 w-4 mr-1" /> PDF
                </Button>
              </div>
            )}
