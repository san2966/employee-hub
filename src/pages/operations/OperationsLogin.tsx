...
const OperationsLogin = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await login(username, password, "operations");
    if (result.success) {
      ...
      <div className="w-full max-w-md animate-scale-in">
        ...
        <div className="bg-card rounded-xl shadow-lg p-8">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-foreground">Operations Login</h1>
            <p className="text-muted-foreground text-sm mt-2">Enter your credentials to access your dashboard</p>
          </div>
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-2">
              <Label htmlFor="username">Username</Label>
              <Input id="username" placeholder="Enter your username" value={username} onChange={(e) => setUsername(e.target.value)} className="h-11" maxLength={50} autoComplete="username" />
            </div>
            <div className="space-y-2">
