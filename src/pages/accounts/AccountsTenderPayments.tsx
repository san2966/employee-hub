...
import { Plus, CreditCard, Download, Trash2, Search, Eye } from "lucide-react";
import { toast } from "@/hooks/use-toast";

const AccountsTenderPayments = () => {
  const { payments, loading, addPayment, deletePayment, payPayment, uploadProof, getProofUrl } = useTenderPayments();
  ...
  const handlePay = async () => {
    if (!payBank || !payDate || !payFile) {
      toast({ title: "Error", description: "Fill all fields and upload proof", variant: "destructive" });
      return;
    }
    setPayLoading(true);
    const path = await uploadProof(payFile);
    if (path) {
      await payPayment(payingId, payBank.trim(), payDate, path);
      setPayOpen(false);
      setPayBank(""); setPayDate(""); setPayFile(null);
    }
    setPayLoading(false);
  };
  ...
  return (
    <AccountsLayout title="Tender Payment Management">
      <div className="space-y-6">
                    ...
                    <TableRow key={p.id}>
                      ...
                      <TableCell>
                        <div className="flex gap-2">
                          ...
                          {!p.paid && (
                            <Button size="sm" variant="default" onClick={() => { setPayingId(p.id); setPayOpen(true); }}>
                          ...
                          <Button size="sm" variant="ghost" className="text-destructive" onClick={() => deletePayment(p.id)}>
        ...
        {/* Pay Dialog */}
        <Dialog open={payOpen} onOpenChange={o => { if (!o) { setPayOpen(false); setPayFile(null); } }}>
          <DialogContent>
            <DialogHeader>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input value={payBank} onChange={e => setPayBank(e.target.value)} placeholder="e.g. ICICI Bank" />
              </div>
              <div>
              ...
              <div>
                <Label>Upload Proof (PDF, max 5MB) *</Label>
                <Input type="file" accept=".pdf" onChange={e => setPayFile(e.target.files?.[0] || null)} />
              </div>
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setPayOpen(false)}>Cancel</Button>
              <Button onClick={handlePay} disabled={payLoading}>
                {payLoading ? "Uploading..." : "Pay"}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
            ...
            {previewRecord && (
              <div className="space-y-3 text-sm">
                <div className="grid grid-cols-2 gap-2">
                  ...
                  {previewRecord.paid_at && <div><span className="text-muted-foreground">Paid At:</span> {new Date(previewRecord.paid_at).toLocaleString()}</div>}
                </div>
...
export default AccountsTenderPayments;
